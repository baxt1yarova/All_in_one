...
for(let i=1;i<=45;i++){
    ...
    div.innerHTML=`
                <img src="photos/${product_json[i]['img']}" class="product-image">
...
let pul=250000000000
let count=document.querySelectorAll('.count')
let k = Array(45).fill(0);
let price=document.querySelectorAll('.price')
let sell=document.querySelectorAll('.sell-button')
document.querySelectorAll('.buy-button').forEach((num,index)=>{
    num.onclick=function(){
        let s=''
        k[index]+=1
        count[index].value=k[index]
        pul-=product_json[index+1]['price']
        sell[index].style.backgroundColor='red'
        sell[index].style.color='white'
        let m=pul
        while(m!=0){
            a1=m%1000
            if(a1<100 && a1>9){
...
document.querySelectorAll('.sell-button').forEach((num,index)=>{
    num.onclick=function(){
        let s=''
        if(k[index]!=0){
            k[index]-=1
            if(k[index]==0){
                sell[index].style.color='#000000'
                sell[index].style.backgroundColor='#cccccc54'
            }
            count[index].value=k[index]
            pul+=product_json[index+1]['price']
            let m=pul
